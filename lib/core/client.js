...
function _resume (client, sync) {
  while (true) {
    ...
    const host = request.host || client[kTLSOpts].servername || client[kUrl].host

    if (client[kUrl].protocol === 'https:' && client[kHost] !== host) {
      if (client.running) {
        return
      }
      ...
      const servername = host.charAt(0) !== '[' && !net.isIP(host) ? host : null
